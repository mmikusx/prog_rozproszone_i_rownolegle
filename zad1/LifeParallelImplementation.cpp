...
void LifeParallelImplementation::mpiSendData(int target, int row, int tag, int **matrix) {
    MPI_Request request;
    MPI_Isend(&matrix[row][0], size, MPI_INT, target, tag, MPI_COMM_WORLD, &request);
    MPI_Wait(&request, MPI_STATUS_IGNORE);
}

void LifeParallelImplementation::mpiReceiveData(int source, int row, int tag, int **matrix) {
    MPI_Request request;
    MPI_Irecv(&matrix[row][0], size, MPI_INT, source, tag, MPI_COMM_WORLD, &request);
    MPI_Wait(&request, MPI_STATUS_IGNORE);
}

void LifeParallelImplementation::sendData(int target, int row, int tag) {
    mpiSendData(target, row, tag, cells);
    mpiSendData(target, row, tag + 1, pollution);
}

void LifeParallelImplementation::receiveData(int source, int row, int tag) {
    mpiReceiveData(source, row, tag, cells);
    mpiReceiveData(source, row, tag + 1, pollution);
}
...
void LifeParallelImplementation::exchangeData() {
    isOdd = (processIdentificator % 2 == 1);
...
void LifeParallelImplementation::prepareNextGeneration() {
    for (int row = startingRowForProcess; row <= endingRowForProcess; row++) {
        ...
        for (int col = 1; col < size_1; col++) {
            ...
            currentPollutionValue = pollution[row][col];

            PollutionValues p = getPollutionValues(pollution, row, col);

            cellsNext[row][col] = rules->cellNextState(currentCellValue, liveNeighbours(row, col),
                                                       currentPollutionValue);
            pollutionNext[row][col] = rules->nextPollution(currentCellValue, currentPollutionValue, p.sumPoll,
                                                           p.sumNsPoll);
        }
    }
}
...
void LifeParallelImplementation::getProcessInfo() {
...
void LifeParallelImplementation::calculateRowDistribution(int si, int numberOfDataRowsPerProcess, int rowsLeft) {
    ...
    endingRowForProcess =
            startingRowForProcess + numberOfDataRowsPerProcess - 1 > si + 1 ? si + 1 : startingRowForProcess +
                                                                                       numberOfDataRowsPerProcess - 1;
}
...
void LifeParallelImplementation::beforeFirstStep() {
    ...
    si = size - 2;
    numberOfDataRowsPerProcess = si / numberOfProcesses;
    rowsLeft = si % numberOfProcesses;
...
void LifeParallelImplementation::sendDataToRoot() {
    isSender = (processIdentificator != 0);
